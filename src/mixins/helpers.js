...
export default {
  methods: {
    generateSecondsMatrix(date) {
      return Array.from(
        { length: 60 },
        (elColumn, columnIndex) => DateTime
          .local(date.year,
            date.month, date.day, date.hour, date.minute, columnIndex),
      );
    },
    generateMinutesMatrix(date, step = 1) {
      return Array.from({ length: Math.ceil(60 / step) },
        (elColumn, columnIndex) => DateTime
          .local(
            date.year,
            date.month,
    ...
    generateYearsMatrix(date) {
      return Array.from({ length: 10 },
        (elColumn, columnIndex) => {
          const dateTime = DateTime.local(
            (date.year - (date.year % 10)) + columnIndex, date.month,
            1,
            date.hour, date.minute, date.second,
          );
    ...
    getPreviousMonth(date) {
      return DateTime.local(
        date.year, date.month, 1, date.hour, date.minute, date.second,
      )
        .minus({ month: 1 });
    },
    getNextMonth(date) {
    ...
    toDecade(date) {
      return DateTime.local(
        date.year - (date.year % 10), date.month, 1, date.hour, date.minute, date.second,
      );
    },
    getNextYear(date) {
      return DateTime.local(
    ...
    getShortNameWeekdays(
      startFromSunday = false, locale = 'en',
    ) {
      return Array.from(
        { length: 7 }, (elColumn, columnIndex) => {
