...
const processDirectiveArguments = (bindingValue) => {
  ...
  return {
    ...
    active: isFunction && bindingValue.active === undefined
...
const destroyInstance = (el) => {
  const instanceIndex = funcIndexOf(instances,
  ...
  instance.eventHandlers.forEach(({ event, handler }) => document
    .removeEventListener(event, handler, true));
...
const update = (el, { value }) => {
  ...
  const instance = instances
    .find(instanceElement => instanceElement.el === el)
    || createInstance(el, events, handler, middleware);
  ...
  instance.eventHandlers
    .forEach(
      ({ event, handler: handlerEvent }) => document
        .removeEventListener(event, handlerEvent, true),
    );
